-- [[ ZORX HUB & i V1: STAFF EDITION + REAL SUPER SPEED + KILL SOUND ]] --
...
local function PlayKillEffect()
    ...
    if char and char:FindFirstChild("HumanoidRootPart") then
        ...
        sound.Volume = 10 -- Volume maksimal engine
...
local function SendStaffChat()
    local staffTexts = {"Server monitoring active.", "Checking for unusual activity.", "Staff monitoring: Server status Clear."}
...
addBtn("🔊 TEST KILL SOUND", zorxScroll, function() PlayKillEffect() end)
...
RunService.RenderStepped:Connect(function()
    ...
    if aimlockOn then
        if not lockTarget or not lockTarget.Parent or lockTarget.Parent.Humanoid.Health <= 0 then lockTarget = getClosest() end
